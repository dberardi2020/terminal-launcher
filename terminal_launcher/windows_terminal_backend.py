...
Two things make Windows different from the iTerm2 path:
  ...
  * No per-pane text API. iTerm2 delivers `/color` with `session.async_send_text`
    ...
    the target window and typing the command (see `_inject_color`).
...
def _wait_title(hwnd: int, name: str, cap: float = 20.0) -> None:
    ...
    This is the readiness gate: snapping/typing before Claude has painted risks a
...
def _uchar(ch: str, up: bool) -> _INPUT:
    flags = _KEYEVENTF_UNICODE | (_KEYEVENTF_KEYUP if up else 0)
    ki = _KEYBDINPUT(wVk=0, wScan=ord(ch), dwFlags=flags, time=0, dwExtraInfo=None)
    return _INPUT(type=_INPUT_KEYBOARD, u=_INPUTUNION(ki=ki))


def _type_text(text: str) -> None:
    """Type `text` a char at a time as Unicode input.

    Dismiss any open autocomplete first (Escape), then send one char per SendInput
    with a small gap so Claude's slash-command autocomplete can't intercept and
    corrupt the command (e.g. eat the space after `/color`)."""
    _send([_vkey(_VK_ESCAPE, False), _vkey(_VK_ESCAPE, True)])
    time.sleep(0.05)
    for ch in text:
        _send([_uchar(ch, False), _uchar(ch, True)])
        time.sleep(0.015)


def _inject_color(hwnd: int, color: str) -> None:
    """Focus the window and type `/color <name>` + Enter."""
    if not _force_foreground(hwnd):
        _log.warning("could not foreground window for /color '%s'; skipping", color)
        return
    _type_text(f"/color {color}")
    time.sleep(0.2)
    _send([_vkey(_VK_RETURN, False), _vkey(_VK_RETURN, True)])  # submit
    time.sleep(0.1)
