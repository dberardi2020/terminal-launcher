...
| [Tech Stack](Tech-Stack.md) | The dependencies — stdlib core, pywebview, iterm2 API, wezterm cli, py2app, pytest — and *why* each. |
...
| [Build, Packaging & Testing](Build-Packaging-Testing.md) | The py2app `.app` bundle, asset loading, entitlements, and the test suite. |

## The shape, at a glance

~1,870 lines of Python across 11 modules, plus one self-contained HTML/JS GUI file.
...
terminal_launcher/
  ...
  model.py                     # workspace → concrete ResolvedSlots; compaction
  layouts.py                   # split-plans (terminal-agnostic; unit-tested)
  backend.py                   # selects the terminal backend per platform
  iterm2_backend.py            # macOS backend — iTerm2 Python API
  wezterm.py                   # non-macOS backend — `wezterm cli`
  assets/wezterm-maximize.lua  # WezTerm gui-startup maximize hook
...
- **Neither front-end knows which terminal it's driving.** They call
  `backend.launch(...)`; the platform split (iTerm2 vs WezTerm) lives entirely behind
  that seam. See [Backends](Backends.md).

## Deeper references

- [`../decisions/`](../decisions/) — the ADRs: *why* each choice was made (start with
  [0001](../decisions/0001-terminal-layer-and-core.md) and
  [0007](../decisions/0007-iterm2-backend-and-real-gap-layouts.md)).
