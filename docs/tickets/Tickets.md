...
| TLA-0019 | Chore | P2 | backend | **Verify `/restore` on Windows Terminal:** `windows_terminal_backend.restore_current` takes the foreground `wt` window and clipboard-pastes `/color` + `/rename`. Detection is shared + unit-tested; the Windows injection path is written but unverified — smoke-test on the PC: the foreground-window assumption, `GetForegroundWindow` HWND restype, and paste timing/ordering of the two commands. Same platform-verification gap as the rest of the Windows backend. See [ADR 0009](../decisions/0009-restore-pane-identity.md). |
