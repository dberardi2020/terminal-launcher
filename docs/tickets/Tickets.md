...
| TLA-0019 | Chore | P2 | backend | **Verify `/restore` on Windows:** (a) `windows_terminal_backend.restore_current` takes the foreground `wt` window and clipboard-pastes `/color` + `/rename` — check the foreground-window assumption, `GetForegroundWindow` HWND restype, and paste timing/ordering of the two commands; (b) the launch-time `_paste_command` extraction didn't regress normal `/color` injection; (c) `integrations/claude-code/install.ps1` generates a working `/restore` command and its invocation line runs in Claude Code's shell on Windows. Detection is shared + unit-tested. See [ADR 0009](../decisions/0009-restore-pane-identity.md). |
